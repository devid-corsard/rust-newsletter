...
pub async fn confirm(parameters: web::Query<Parameters>, pool: web::Data<PgPool>) -> HttpResponse {
    let subscriber_id =
        match get_subscriber_id_by_token(&pool, &parameters.subscription_token).await {
            Ok(id) => id,
...
#[tracing::instrument(name = "Get subscriber_id by token", skip(pool, subscription_token))]
